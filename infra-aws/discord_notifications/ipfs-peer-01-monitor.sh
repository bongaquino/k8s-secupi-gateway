...
DISCORD_USERNAME="🔗 IPFS Peer-01 Monitor"
DISCORD_AVATAR_URL="https://ipfs.io/ipfs/QmYjtig7VJQ6XsnUjqqJvj7QaMcCAwtrgNdahSiFofrE7o"

# Monitoring Configuration
LOG_FILE="/tmp/ipfs-peer-01-monitor.log"
STATE_DIR="/tmp/ipfs-peer-01-monitoring"
TIMEOUT=30
...
is_local_execution() {
    ...
    [ "$current_ip" = "$SERVER_IP" ] || [ "$(hostname)" = "peer-01" ] || [ "$USER" = "$SERVER_USER" ]
}
...
test_ssh_connectivity() {
    ...
    if is_local_execution; then
        log_message "Running locally on Peer-01"
        return 0
    fi
...
check_system_resources() {
    ...
    # Disk Usage - Check data disk for Peer-01 (14.6TB storage)
    local disk_usage=$(ssh_execute "df -h /data | tail -1 | awk '{print \$5}' | sed 's/%//'" 2>/dev/null)
    if [ -z "$disk_usage" ]; then
        # Fallback to root disk if /data not mounted
        disk_usage=$(ssh_execute "df -h / | tail -1 | awk '{print \$5}' | sed 's/%//'" 2>/dev/null || echo "unknown")
    fi
    ...
    local details="CPU: ${cpu_usage}%, Memory: ${memory_usage}%, Data disk: ${disk_usage}% (14.6TB expandable)"
...
check_storage_health() {
    ...
    # Check data disk space (14.6TB XFS filesystem)
    local data_disk_usage=$(ssh_execute "df -h /data | tail -1 | awk '{print \$5}' | sed 's/%//'" 2>/dev/null || echo "unknown")
    local data_disk_info=""
    if [ "$data_disk_usage" != "unknown" ]; then
        data_disk_info="Data disk: ${data_disk_usage}%"
    else
        data_disk_info="Data disk: not mounted"
    fi
    ...
    local details="IPFS repo: $repo_status, $data_disk_info, Storage errors: $storage_errors (14.6TB expandable)"
...
run_all_checks() {
    log_message "=== Starting IPFS Peer-01 health checks ==="
    ...
    if ! test_ssh_connectivity; then
        send_critical_alert "SSH Connectivity Failed" "Cannot connect to IPFS peer node" "Server: $SERVER_IP, User: $SERVER_USER"
        return 1
    fi
...
send_health_summary() {
    ...
    # Get system metrics
    local cpu_usage=$(ssh_execute "top -bn1 | grep 'Cpu(s)' | awk '{print \$2}' | sed 's/%us,//' | cut -d'.' -f1" 2>/dev/null || echo "unknown")
    ...
    summary_details="**System Resources:** $system_state (CPU: ${cpu_usage}%, Memory: ${memory_usage}%, Data disk: ${disk_usage}%)\n"
    ...
    summary_details="${summary_details}**Storage:** $storage_state (14.6TB expandable storage)"
    
    send_info_alert "IPFS Peer-01 Health Summary" "Daily health report" "$summary_details"
}
...
restart_ipfs_services() {
    ...
    # Restart IPFS daemon
    if ssh_execute "cd /home/ipfs/example-ipfs/docker-compose/example-ipfs-node-peer-01 && docker-compose restart ipfs"; then
    ...
    # Restart cluster
    if ssh_execute "cd /home/ipfs/example-ipfs/docker-compose/example-ipfs-node-peer-01 && docker-compose restart ipfs-cluster"; then
...
main() {
    ...
    case "${1:-check}" in
        "check")
            log_message "=== Starting IPFS Peer-01 monitoring ==="
            run_all_checks
            ;;
        "summary")
            send_health_summary
        ...
        "test")
            send_discord_alert "🧪 Test Alert" \
                "Testing IPFS Peer-01 monitoring integration" \
                3447003 \
        ...
        "cleanup")
            ...
            ;;
        "setup")
            log_message "Setting up IPFS Peer-01 monitoring..."
            ...
            # Add to crontab
            (crontab -l 2>/dev/null | grep -v "ipfs-peer-01-monitor.sh"; echo "*/5 * * * * $(realpath $0) check") | crontab -
            (crontab -l 2>/dev/null | grep -v "ipfs-peer-01-monitor.sh"; echo "0 8 * * * $(realpath $0) summary") | crontab -
            
            log_message "IPFS Peer-01 monitoring setup complete!"
            send_success_alert "IPFS Peer-01 Monitoring Started" \
