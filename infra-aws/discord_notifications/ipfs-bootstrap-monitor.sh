...
DISCORD_USERNAME="🔗 IPFS Bootstrap Monitor"
DISCORD_AVATAR_URL="https://ipfs.io/ipfs/QmYjtig7VJQ6XsnUjqqJvj7QaMcCAwtrgNdahSiFofrE7o"

# Monitoring Configuration
LOG_FILE="/tmp/ipfs-bootstrap-monitor.log"
STATE_DIR="/tmp/ipfs-bootstrap-monitoring"
TIMEOUT=30
...
# Detect if we're running locally on the bootstrap node
is_local_execution() {
    # Check multiple ways to detect if we're on the bootstrap node
    local current_ip=""
    ...
    [ "$current_ip" = "$SERVER_IP" ] || [ "$(hostname)" = "bootstrap-node" ] || [ "$USER" = "$SERVER_USER" ]
}
...
test_ssh_connectivity() {
    ...
    if is_local_execution; then
        log_message "Running locally on Bootstrap Node"
        return 0
    fi
...
check_system_resources() {
    ...
    # Disk Usage
    local disk_usage=$(ssh_execute "df -h / | tail -1 | awk '{print \$5}' | sed 's/%//'" 2>/dev/null)
    ...
    local details="CPU: ${cpu_usage}%, Memory: ${memory_usage}%, Disk: ${disk_usage}%"
...
check_storage_health() {
    ...
    local disk_usage=$(ssh_execute "df -h / | tail -1 | awk '{print \$5}' | sed 's/%//'" 2>/dev/null || echo "unknown")
    ...
    if [ "$disk_usage" != "unknown" ] && [ "$disk_usage" -gt "$DISK_THRESHOLD" ]; then
        alerts="${alerts}Root disk usage: ${disk_usage}% "
        status="critical"
    fi
    ...
    local details="IPFS repo: $repo_status, Root disk: ${disk_usage}%, Storage errors: $storage_errors"
...
run_all_checks() {
    log_message "=== Starting IPFS Bootstrap Node health checks ==="
    ...
    if ! test_ssh_connectivity; then
        send_critical_alert "SSH Connectivity Failed" "Cannot connect to IPFS bootstrap node" "Server: $SERVER_IP, User: $SERVER_USER"
        return 1
    fi
...
send_health_summary() {
    ...
    summary_details="${summary_details}**Storage:** $storage_state"
    
    send_info_alert "IPFS Bootstrap Health Summary" "Daily health report" "$summary_details"
}
...
restart_ipfs_services() {
    ...
    # Restart IPFS daemon
    if ssh_execute "cd /home/ipfs/koneksi-ipfs/docker-compose/koneksi-ipfs-kr-bootstrap-02 && docker-compose restart ipfs"; then
    ...
    # Restart cluster
    if ssh_execute "cd /home/ipfs/koneksi-ipfs/docker-compose/koneksi-ipfs-kr-bootstrap-02 && docker-compose restart ipfs-cluster"; then
...
main() {
    ...
    case "${1:-check}" in
        "check")
            log_message "=== Starting IPFS Bootstrap Node monitoring ==="
            run_all_checks
            ;;
        "summary")
            send_health_summary
        ...
        "test")
            send_discord_alert "🧪 Test Alert" \
                "Testing IPFS Bootstrap Node monitoring integration" \
                3447003 \
        ...
        "cleanup")
            ...
            ;;
        "setup")
            log_message "Setting up IPFS Bootstrap Node monitoring..."
            ...
            # Add to crontab
            (crontab -l 2>/dev/null | grep -v "ipfs-bootstrap-monitor.sh"; echo "*/5 * * * * $(realpath $0) check") | crontab -
            (crontab -l 2>/dev/null | grep -v "ipfs-bootstrap-monitor.sh"; echo "0 8 * * * $(realpath $0) summary") | crontab -
            
            log_message "IPFS Bootstrap Node monitoring setup complete!"
            send_success_alert "IPFS Bootstrap Monitoring Started" \
