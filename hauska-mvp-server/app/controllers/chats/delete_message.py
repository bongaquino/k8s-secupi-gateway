...
async def handle(request: Request, container):
    ...
    # Use DeltaService to delete the image file associated with the chat
    delta_service = container.resolve("services.delta_service")

    chat = chat_service.get_chat_by_message_id(message_id)
    if not chat:
        return FormatHelper.format_response(
            "error",
            404,
            f"Chat with message ID {message_id} not found",
            None,
            None
        )
    ...
    if chat["user_id"] != user_id:
        return FormatHelper.format_response(
            "error",
            403,
            f"Chat with message ID {message_id} does not belong to the user",
            None,
            None
        )
    
    try:
        delta_service.delete_image(chat["delta_id"])
    except Exception as e:
        return FormatHelper.format_response(
            "error",
            500,
            f"Failed to delete image file for message ID {message_id}: {str(e)}",
            None,
            None
        )
