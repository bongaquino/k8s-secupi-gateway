...
async def handle(request: Request, container):
    ...
    # Use DeltaService to get image URLs for each chat
    delta_service = container.resolve("services.delta_service")
    for chat in chats:
        if chat.get("delta_id"):
            try:
                image_url = delta_service.get_image_url(chat["delta_id"])
                chat["image_url"] = image_url
            except Exception as e:
                return FormatHelper.format_response(
                    "error",
                    500,
                    f"Failed to get image URL for chat {chat['delta_id']}: {str(e)}",
                    None,
                    None
                )
        # Remove delta_id from chat
        del chat["delta_id"]
