...
async def handle(request: Request, container):
    ...
    if content_type is None:
        ...
        file = {
            "file": (image.filename, image.file.read(), image.content_type)
        }
        ...
        # Use DeltaService to upload the image
        try:
            delta_service = container.resolve("services.delta_service")
            delta_id = delta_service.upload_image(file)
        except Exception as e:
            return FormatHelper.format_response(
                "error",
                500,
        ...
        chat_service = container.resolve("services.chat_service")
        try:
            response_message = chat_service.process_and_save_message(user_id, query_message, delta_id, base64_image)
        except Exception as e:
            return FormatHelper.format_response(
                "error",
                500,
